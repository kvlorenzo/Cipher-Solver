...
function testMe2() {
	...
	document.getElementById('outputbox').innerHTML = isNaN(shiftVal) ? caesar(message, 7) : caesar(message, shiftVal);
...
function togSteps() {
	var message = document.getElementById('messagebox').value;
	var shiftVal = parseInt(document.getElementById('shiftVal').value);
	document.getElementById('steps').innerHTML = cipher.showSteps(message);
}